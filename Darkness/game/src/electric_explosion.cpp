...
electric_explosion::electric_explosion(glm::vec3 start_colour, glm::vec3 end_colour, bool zap, float zap_level, float max_time)
{
...
void electric_explosion::on_update(const engine::timestep& time_step)
{
	if (!s_active)
		return;
...
void electric_explosion::on_render(engine::ref<engine::shader> shader)
{
	if (!s_active)
		return;
...
void electric_explosion::activate(float radius, glm::vec3 position)
{
	s_active = true;
	m_position = position;
	m_timer = 0.0f;
...
engine::ref<electric_explosion> electric_explosion::create(glm::vec3 start_colour, glm::vec3 end_colour, bool zap, float zap_level, float max_time)
{
	return std::make_shared<electric_explosion>(start_colour, end_colour, zap, zap_level, max_time);
}

// Return a random floating point number between 0 and 1
float electric_explosion::random()
{
